...
class CatalogoEspecies:
    def __init__(self, root):
        ...
        self.label_nome = ctk.CTkLabel(root, text="Nome da Espécie:")
        self.label_nome.place(x=20, y=20)
        self.entry_nome = ctk.CTkEntry(root, width=200)
        self.entry_nome.place(x=150, y=20)

        self.label_status = ctk.CTkLabel(root, text="Nível de Extinção:")
        ...
        self.label_imagem = ctk.CTkLabel(root, text="Imagem:")
        self.label_imagem.place(x=20, y=100)
        self.button_imagem = ctk.CTkButton(root, text="Carregar Imagem", command=self.carregar_imagem)
        self.button_imagem.place(x=150, y=100)

        self.label_descricao = ctk.CTkLabel(root, text="Descrição:")
        ...
        self.button_adicionar = ctk.CTkButton(root, text="Adicionar/Salvar Espécie", command=self.adicionar_especie)
        self.button_adicionar.place(x=20, y=250)
        self.button_remover = ctk.CTkButton(root, text="Remover Espécie", command=self.remover_especie)
        self.button_remover.place(x=195, y=250)
        self.button_buscar = ctk.CTkButton(root, text="Buscar Espécie", command=self.buscar_especie)
        self.button_buscar.place(x=358, y=250)
        self.button_buscar = ctk.CTkButton(root, text="Limpar", command=self.limpar_dados)
        self.button_buscar.place(x=550, y=250)
        # Treeview para listar espécies
        self.treeview_especies = ttk.Treeview(root, columns=("Nome", "Descrição", "Status"), show="headings", height=10)
